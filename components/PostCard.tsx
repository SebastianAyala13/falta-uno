...
export default function PostCard({ post, comentarios = 0 }: PostCardProps) {
  ...
  return (
    <Pressable
      ...
      style={{ borderColor: esRecap ? Colors.accent + '66' : Colors.border }}>
      {/* Header */}
      <View className="flex-row items-center gap-3 px-4 pt-4">
        ...
        <View
          ...
          style={{ backgroundColor: esRecap ? Colors.accent + '22' : Colors.background }}>
          <Ionicons name={meta.icon} size={12} color={esRecap ? Colors.accent : Colors.primary} />
          <Text
            className="font-body-semibold text-[11px] uppercase tracking-wide"
            style={{ color: esRecap ? Colors.accent : Colors.primary }}>
            {meta.label}
          </Text>
        </View>
        <ModeracionBoton
      ...
      <View className="mt-3 flex-row items-center gap-5 border-t border-border px-4 py-3">
        <Pressable onPress={onLike} hitSlop={8} className="flex-row items-center gap-1.5">
          ...
          />
          <Text className="font-body-semibold text-sm" style={{ color: liked ? Colors.danger : Colors.muted }}>
            {post.likes.length}
          </Text>
        </Pressable>
